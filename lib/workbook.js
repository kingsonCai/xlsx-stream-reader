          ...
          default:
            if ((match = entry.path.match(/xl\/(worksheets\/sheet(\d+)\.xml)/i))) {
              ...
              if (self.parsedWorkBookInfo === false ||
                ...
                let tmpOption = self.options.tmpOption || {}
                tmpOption.tmpdir = tmpOption.tmpdir || Path.resolve(process.cwd(), '~tmp')
                if(!Fs.existsSync(tmpOption.tmpdir)) Fs.mkdirSync(tmpOption.tmpdir)
                const { name } = Tmp.fileSync(tmpOption)
